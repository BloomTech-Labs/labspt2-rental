...
export const PropertyStats = (props) => {
    let percentage;
    if(props.propTotal){
        percentage = Math.floor((props.noReservations.length/props.propTotal)*100)
    }
    return (
        <Fragment>
            <Properties percentage={percentage} propTotal={props.propTotal} noReservations={props.noReservations} mobile={props.mobile}/>
        </Fragment>
    );
  }
    ...
    const { propTotal, noReservations, percentage } = props;
      ...
      if (!propTotal){
        ...
        properties = <ZeroProperties />
          ...
          properties = <OneProperty percentage={percentage} propTotal={propTotal} noReservations={noReservations} />
      }
...
const ZeroProperties = () => {
    return (
        <FlexColumn alignCenter>
            ...
            <Header as='h4' style={{marginTop: '0.5em', marginBottom: 0}}>No properties to show.</Header>
            <Link to='/dashboard/properties/add' style={{ marginTop: '2em'}}>
                <Button basic color='blue'>Add New Property</Button>
            </Link>
        </FlexColumn>
    )
}
...
const OneProperty = (props) => {
    ...
    if (props.percentage > 50) {
        icon = ( <FlexRow style={{width: '70%', alignItems: 'baseline', justifyContent: 'flex-start', marginLeft: '1%', marginTop: '2%'}}>
        <Icon name='exclamation triangle' color='red' size='large' />
        <FlexRow style={{ marginLeft: '5%' }}>
            <p style={{fontSize: '1.2em'}}><strong>{props.percentage}%</strong> are underbooked</p>
            <Popup
                trigger={<Icon name='info' size='small' style={{ alignSelf: 'flex-start', opacity: '0.8', paddingLeft: '4%' }} />}
        ...
        icon = ( <FlexRow style={{width: '70%', alignItems: 'baseline', justifyContent: 'space-between', marginLeft: '1%'}}>
        <Icon name='exclamation triangle' color='yellow' size='large' />
        <p style={{fontSize: '1.2em'}}><strong>{props.percentage}%</strong> are underbooked</p>
    </FlexRow>)
    } else {
        icon = ( <FlexRow style={{width: '80%', alignItems: 'center', justifyContent: 'space-between', marginLeft: '1%'}}>
        <Icon name='checked calendar' color='green' size='large' />
        <p style={{fontSize: '1.2em'}}><strong>All your properties are performing well!</strong></p>
    </FlexRow>)
    }

    let buttons;
    if(Object.keys(props.noReservations).length === 1) {
        buttons = (<FlexRow style={{ marginBottom: '1em', marginTop: '2em'}}>
        ...
        buttons = (<Link to='/dashboard/properties' style={{ marginTop: '2em'}}>
        <Button basic color='blue'>View Properties</Button>
        </Link>)
    } else {
        buttons = (<FlexRow style={{ marginBottom: '1em', marginTop: '2em'}}>
        <Link to={`/dashboard/properties/view/${props.noReservations[0]._id}`}>
